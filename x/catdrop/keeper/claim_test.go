...
func (suite *KeeperTestSuite) TestHookBeforeAirdropStart() {
	...
	claimRecords := []types.ClaimRecord{
		{
			...
			InitialClaimableAmount: sdk.NewCoins(sdk.NewInt64Coin(types.DefaultClaimDenom, 100000)),
	...
	suite.app.CatdropKeeper.AfterProposalVote(suite.ctx.WithBlockTime(airdropStartTime), 1, addr1)
	// suite.app.CatdropKeeper.AfterDelegationModified(suite.ctx.WithBlockTime(airdropStartTime.Add(time.Minute * 3)), addr1, sdk.ValAddress(addr1))
	balances = suite.app.BankKeeper.GetAllBalances(suite.ctx, addr1)
	
	suite.Equal(
		claimRecords[0].InitialClaimableAmount.AmountOf(types.DefaultClaimDenom).Quo(
			sdk.NewInt(int64(len(types.Action_value)))),
...
func (suite *KeeperTestSuite) TestAirdropDisabled() {
	...
	// Now, it is the time for air drop, so claim module should send the balances to the user after delegate.
	suite.Equal(
		claimRecords[0].InitialClaimableAmount.AmountOf(types.DefaultClaimDenom).Quo(sdk.NewInt(int64(len(types.Action_value)))),
		balances.AmountOf(types.DefaultClaimDenom),
	)
}
